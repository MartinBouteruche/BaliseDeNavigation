...
void ST7036::lcd_print(uint8_t value) 
{
   // If the LCD has been initialised correctly write to it
   // -----------------------------------------------------
   if ( _initialised )
...
void ST7036::lcd_print(const uint8_t *buffer, size_t size)
{
   // If the LCD has been initialised correctly, write to it
   // ------------------------------------------------------
   if ( _initialised )
...
void ST7036::load_custom_character (uint8_t char_num, uint8_t *rows)
{
   ...
   if ( _initialised )
   {
      ...
      if ( char_num < MAX_USER_CHARS )
      {
         ...
         if ( _status == 0 )
         {
            lcd_print ( rows, PIXEL_ROWS_PER_CHAR ); // write the character to CGRAM 
